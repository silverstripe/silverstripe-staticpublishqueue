...
summary: Configuring the Static Publish Queue module for your project.
icon: cog
---

# Basic configuration

By default, the extensions [`SiteTreePublishingEngine`](api:SilverStripe\StaticPublishQueue\Extension\Engine\SiteTreePublishingEngine) and `PublishableSiteTree`
are applied to [`SiteTree`](api:SilverStripe\CMS\Model\SiteTree), so as soon as the module is installed, it should
be ready to go.
...
Which will ensure that the [`GenerateStaticCacheJob`](api:SilverStripe\StaticPublishQueue\Job\GenerateStaticCacheJob)s are processed quickly.
...
You can exclude pages from being statically generated on a class-by-class basis by adding a [`StaticallyPublishable::urlsToCache()`](api:SilverStripe\StaticPublishQueue\Contract\StaticallyPublishable::urlsToCache()) method to your page class which returns an empty array:
...
By default, static files are generated for all page responses regardless of the response's status code.
If you would like to exclude certain status codes from being statically cached, set the [`FilesystemPublisher.disallowed_status_codes`](api:SilverStripe\StaticPublishQueue\Publisher\FilesystemPublisher->disallowed_status_codes) configuration property:
...
behaviour and have [`SiteTree.enforce_strict_hierarchy`](api:SilverStripe\CMS\Model\SiteTree->enforce_strict_hierarchy) set to `true`, then this means that parent pages must be published for
...
This module comes with two essential interfaces: [`StaticPublishingTrigger`](api:SilverStripe\StaticPublishQueue\Contract\StaticPublishingTrigger) and [`StaticallyPublishable`](api:SilverStripe\StaticPublishQueue\Contract\StaticallyPublishable). These interfaces
...
republishing or URLs assigned to other objects, implement the `StaticPublishingTrigger`. In our example of `Page`
...
- [`SiteTreePublishingEngine`](api:SilverStripe\StaticPublishQueue\Extension\Engine\SiteTreePublishingEngine) - regenerates the static cache files after a page is published/unpublished. This makes use
of both interfaces as object trigger refresh of other objects.

### Example: triggering a republish of the homepage after an update to a `DataObject` model

In order for the triggers to get picked up and executed, make sure to add the SiteTreePublishingEngine to your DataObject.

```yml
App\Models\YourDataObject:
  extensions:
...
class YourDataObject extends DataObject implements StaticPublishingTrigger
{
    ...
    {
        // return 1 or an array of multiple publishable objects you would
        // like to regenerate the static cache for
        return Page::get()->First();
    }

    public function objectsToDelete($context)
    {
      // Return 1 or an array of publishable objects for which you would like to purge the statically cached variant
    }
}
```
...
This functionality can be enabled in the module by adding a configuration setting to your project, e.g.:
